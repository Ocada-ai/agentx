...
export function Stock({ name = "DOGE", price = 12.34, delta = 1 }) {
  ...
  return (
    <div className="p-4 text-yellow-400 bg-[#1a1a1a] border-2 border-[#1f1f1f] shadow-[0_2px_4px_0_#0000001a] rounded-xl font-gabarito">
      {/* <div className="inline-block float-right px-2 py-1 text-xs rounded-full bg-white/10">
        {`${delta > 0 ? '+' : ''}${((delta / price) * 100).toFixed(2)}% ${
          delta > 0 ? '↑' : '↓'
        }`}
      </div> */}
      <div className="text-lg text-zinc-300 text-opacity-60">{name}</div>
      <div className="text-3xl text-zinc-300 font-normal">${price}</div>
      <div className="mt-1 text-xs text text-zinc-300 text-opacity-80">
        ...
        >
          <defs>
            <linearGradient
              ...
              y2="100%"
            >
              <stop offset="0%" stopColor="#f79139" stopOpacity="0.38"></stop>
              <stop offset="13%" stopColor="##ffc1073d" stopOpacity="0"></stop>
            </linearGradient>
